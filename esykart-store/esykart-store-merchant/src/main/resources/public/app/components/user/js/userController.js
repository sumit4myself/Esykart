		...
		[
				...
				function($scope, $rootScope, $timeout, utils, UserService) {
					...
					$scope.validateUserDetails = function(context) {
						...
						var result = parsleyForm.validate({
							group : 'block1',
							force : true
						})
					...
					$scope.saveUser = function() {
						var form_serialized = JSON.stringify(utils
								.serializeObject($user_form), null, 2);
						UIkit.modal.alert('<p>Wizard data:</p><pre>'
								+ form_serialized + '</pre>');
...
.controller(
		...
		function($compile, $scope, $timeout, DTOptionsBuilder,
			...
			vm.dtOptions = DTOptionsBuilder.fromSource('data/dt_data.json')
					...
					}).withOption('bProcessing', true).withOption(
							'bServerSide', true).withOption("aLengthMenu",
